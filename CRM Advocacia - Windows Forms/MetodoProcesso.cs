...
namespace CRM_Advocacia___Windows_Forms
{
    ...
    public class MetodoProcesso
    {
        ...
        public bool AdicionarProcesso(string numero, string titulo, string descricao, string tipo, string data, string nomecliente, string nomeadv,  decimal valorproces, decimal percHonor, string deschonor)
        {
            ...
            using (var transaction = conn.BeginTransaction())
            {
                try
                {
                    ...
                    {
                        cmdInsertProc.Parameters.AddWithValue("@numero", numero);
                        cmdInsertProc.Parameters.AddWithValue("@titulo", titulo);
                        cmdInsertProc.Parameters.AddWithValue("@descricao", descricao);
                        cmdInsertProc.Parameters.AddWithValue("@area", tipo);
                        cmdInsertProc.Parameters.AddWithValue("@idCliente", idCliente);
                        cmdInsertProc.Parameters.AddWithValue("@idAdvogado", idAdvogado);
                        cmdInsertProc.Parameters.AddWithValue("@valor", valorproces);
                        cmdInsertProc.Parameters.AddWithValue("@data", DateTime.Parse(data));

                        idProcesso = Convert.ToInt32(cmdInsertProc.ExecuteScalar());
                    }
                    ...
                    {
                        cmdHonor.Parameters.AddWithValue("@idProcesso", idProcesso);
                        cmdHonor.Parameters.AddWithValue("@descricao", deschonor);
                        cmdHonor.Parameters.AddWithValue("@valor", valorHonorarioCalculado);
                        cmdHonor.Parameters.AddWithValue("@dataEmissao", DateTime.Now.Date);
                        cmdHonor.Parameters.AddWithValue("@pago", false);
                ...
                catch (Exception ex)
                {
                    ...
                    MessageBox.Show("Erro ao adicionar processo: " + ex.Message,
        ...
        {
            string sqlLog = @"INSERT INTO LogOperacoes (operacao, tabela_afetada, id_registro, descricao) VALUES (@operacao, @tabelaAfetada, @idRegistro, @descricao);";
